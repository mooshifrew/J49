 ...
 * @version $Revision$
 */
public class OriginalDistribution implements Distribution {
   ...
   */
  public OriginalDistribution(int numBags, int numClasses) {
   ...
   */
  public OriginalDistribution(double[][] table) {
   ...
   */
  public OriginalDistribution(Instances source) throws Exception {
  ...
  public OriginalDistribution(Instances source, ClassifierSplitModel modelToUse) throws Exception {
   ...
   * distribution.
   */
  public OriginalDistribution(Distribution toMerge) {
    int numClasses = toMerge.numClasses();

    totaL = toMerge.total();
    m_perClass = new double[numClasses];
    for(int c = 0; c<numClasses; c++){
      m_perClass[c] = toMerge.perClass(c);
    }
    m_perClassPerBag = new double[1][0];
    m_perClassPerBag[0] = new double[toMerge.numClasses()];
    System.arraycopy(m_perClass, 0, m_perClassPerBag[0], 0,
      toMerge.numClasses());
    m_perBag = new double[1];
    m_perBag[0] = totaL;
  }
   ...
   * indicated one.
   */
  public OriginalDistribution(Distribution toMerge, int index) {
    ...
    for(i = 0; i<toMerge.numClasses(); i++){
      m_perClass[i] = toMerge.perClass(i);
    }
    m_perClassPerBag = new double[2][0];
    m_perClassPerBag[0] = new double[toMerge.numClasses()];
    m_perClassPerBag[1] = new double[toMerge.numClasses()];
    for (i = 0; i < toMerge.numClasses(); i++) {
      m_perClassPerBag[0][i] = toMerge.perClassPerBag(index, i);
      m_perClassPerBag[1][i] = toMerge.perClass(i) - m_perClassPerBag[0][i];
    }
  ...
  public final Object clone() throws CloneNotSupportedException {
    ...
    OriginalDistribution newDistribution = new OriginalDistribution(m_perBag.length,
      m_perClass.length);

    newDistribution.m_perClassPerBag = Arrays.copyOf(m_perClassPerBag, m_perClassPerBag.length);
    newDistribution.m_perBag = Arrays.copyOf(m_perBag, m_perBag.length);
    newDistribution.m_perClass = Arrays.copyOf(m_perClass, m_perClass.length);
  ...
  public final void initialize() {
    ...
    Arrays.fill(m_perClass, 0);
    Arrays.fill(m_perBag, 0);
  ...
  public final Distribution subtract(Distribution toSubtract) {
    ...
    OriginalDistribution newDist = new OriginalDistribution(1, m_perClass.length);
