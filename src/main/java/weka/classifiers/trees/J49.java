...
import weka.classifiers.trees.j49.BinC45ModelSelection;
import weka.classifiers.trees.j49.C45ModelSelection;
import weka.classifiers.trees.j49.C45PruneableClassifierTree;
import weka.classifiers.trees.j49.ClassifierTree;
import weka.classifiers.trees.j49.DistributionFactory;
import weka.classifiers.trees.j49.ModelSelection;
import weka.classifiers.trees.j49.PruneableClassifierTree;
 ...
 * <!-- options-end -->
 *
 * @author Michael Frew (michael.frew@uwaterloo.ca)
 * @version $Revision$
 */
public class J49 extends AbstractClassifier implements OptionHandler, Drawable,
    ...
    protected DistributionFactory m_distributionFactory;
     ...
     * @param distributionType the type of distribution to be used in the model
     * @throws Exception if classifier can't be built successfully
     */
    public void buildClassifier(Instances instances, int distributionType) throws Exception {
        m_distributionFactory = new DistributionFactory(distributionType);
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        if(m_distributionFactory==null){
            m_distributionFactory = new DistributionFactory();
        }
        ...
        if (m_binarySplits) {
            modSelection = new BinC45ModelSelection(m_minNumObj, instances,
                    m_useMDLcorrection, m_doNotMakeSplitPointActualValue, m_distributionFactory);
        } else {
        ...
        if (!m_reducedErrorPruning) {
            m_root = new C45PruneableClassifierTree(modSelection, !m_unpruned, m_CF,
                    m_subtreeRaising, !m_noCleanup, m_collapseTree, m_distributionFactory);
        } else {
            m_root = new PruneableClassifierTree(modSelection, !m_unpruned,
                    m_numFolds, !m_noCleanup, m_Seed, m_distributionFactory);
        }
    ...
    public String toString() {
        ...
        if (m_unpruned) {
            return "J49 unpruned tree\n------------------\n" + m_root.toString();
        } else {
            return "J49 pruned tree\n------------------\n" + m_root.toString();
        }
    }
    ...
    /**
     * Calculates and returns the average distribution density in the entire tree (as a percentage)
     * @return average distribution density in the tree
     */
    public double getModelDensity() throws Exception {

        double totalDensity = getTotalDensity(m_root);
        int totalNodes = m_root.numNodes();

        //System.out.println("Total Density Percent: " + totalDensity);
        //System.out.println("Total number of Nodes: " + totalNodes);
        return totalDensity / totalNodes;

    }

    /**
     * Recursive function to sum the density of every node in Classifier Tree
     * @param node the root of the tree to recurse through
     * @return total percentage density of tree
     * @throws Exception
     */
    private double getTotalDensity(ClassifierTree node) throws Exception {

        if (node.isLeaf()) {
            return node.getLocalModel().distribution().getDensity();
        }

        double sonsTotal = 0;
        for (int i = 0; i < node.getSons().length; i++) {
            sonsTotal += getTotalDensity(node.getSons()[i]);
        }

        return node.getLocalModel().distribution().getDensity() + sonsTotal;
    }
